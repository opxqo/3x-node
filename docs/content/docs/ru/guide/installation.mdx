---
title: Установка
description: Установите 3x-node с помощью официального скрипта (стабильная, фиксированная или dev-сборка), в автоматическом режиме / через cloud-init либо в Docker — и выберите SQLite или PostgreSQL.
icon: Download
---

3x-node работает на широком спектре дистрибутивов Linux — Ubuntu, Debian, Armbian,
...
bash <(curl -Ls https://raw.githubusercontent.com/opxqo/3x-node/main/install.sh) v3.4.0
```
...
[страницы релизов](https://github.com/opxqo/3x-node/releases), распаковать его и
...
```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/opxqo/3x-node/main/install.sh)
```

В каталоге [`deploy/`](https://github.com/opxqo/3x-node/tree/main/deploy)
