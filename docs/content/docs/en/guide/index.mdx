...
**3x-node** is a web control panel that sits on top of
...
<Mermaid
  chart={`
flowchart LR
  Admin["Admin browser"] -->|HTTPS panel| Panel["3x-node panel"]
...
3x-node is aimed at anyone running their own Xray server: from a single personal
...
<Callout type="info">
  3x-node is an enhanced fork of the original X-UI project, adding broader protocol
