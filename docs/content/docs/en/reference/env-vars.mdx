...
description: Complete reference for 3x-node's XUI_* environment variables — database, panel, logging, memory, node token encryption, and the tunnel health monitor.
icon: Variable
---

3x-node reads its runtime configuration from `XUI_*` environment variables. On a
