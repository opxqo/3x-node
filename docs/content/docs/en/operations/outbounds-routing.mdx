...
description: Shape egress in 3x-node — WARP, NordVPN, PIA WireGuard, outbound subscriptions, routing rules, and load balancers.
icon: Route
---

Inbounds accept clients; **outbounds** decide where their traffic goes next.
3x-node can route traffic through Cloudflare WARP, NordVPN, Private Internet Access
...
shapes 3x-node uses: **VLESS is the flat form** (`address`/`port`/`id`/`flow`/
...
WARP lets your server egress through Cloudflare's network. 3x-node can register a
...
From the panel's WARP controls, register an account. 3x-node fills the outbound's
...
3x-node can fetch NordVPN (NordLynx/WireGuard) credentials from an access token (or
...
3x-node can sign in with a PIA username and password, list countries/regions/servers
