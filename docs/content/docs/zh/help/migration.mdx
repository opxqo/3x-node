---
title: 迁移
description: 从 x-ui 迁移到 3x-node、在服务器之间迁移，或在不同 3x-node 版本之间迁移。
icon: ArrowRightLeft
---
...
[版本发布说明](https://github.com/opxqo/3x-node/releases)
并做好备份。

## 从 x-ui 迁移

将旧版 x-ui 面板的数据库直接导入 3x-node **不受支持**——两者的数据库结构不同。请全新安装 3x-node 并重新创建入站/客户端，同时从旧面板逐一导出分享链接。
