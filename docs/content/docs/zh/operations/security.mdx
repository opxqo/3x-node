---
title: 安全加固
description: 加固 3x-node——面板认证与 2FA、Fail2ban IP 限制、防火墙规则、BBR，以及保持更新。
icon: ShieldCheck
---
...
IP 数量。限制的执行由 **Fail2ban** 负责，3x-node 会为你安装并配置好它（脚本安装时默认启用，
...
定期更新 3x-node 和 Xray-core——安全修复会随新版本发布。请关注
[发布页面](https://github.com/opxqo/3x-node/releases)，并参见
