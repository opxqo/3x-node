# 3x-node on Hetzner Cloud

Hetzner Cloud does **not** have a third-party image marketplace the way AWS does.
Ship 3x-node via **cloud-init**: each instance installs non-interactively and
...
3x-node non-interactively and generates unique per-instance credentials.
...
is essentially a documented cloud-init config plus metadata. To propose 3x-node as
