...
for arch in amd64 arm64; do
	...
	printf '3x-ui-node\n0.1.18-node\n%s\n26.7.28\n' "$arch" > "$dir/manifest"
	archive="$out/3x-ui-node-0.1.18-linux-$arch.tar.gz"
	COPYFILE_DISABLE=1 tar --no-xattrs -czf "$archive" -C "$dir" 3x-ui-node xray service install.sh LICENSE XRAY-LICENSE README.md VALIDATION.md manifest
